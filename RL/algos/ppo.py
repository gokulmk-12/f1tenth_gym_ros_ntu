...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        STEER_MIN, STEER_MAX = -0.34, 0.34
        SPEED_MIN, SPEED_MAX = 0.0, 5.0

        self.register_buffer("action_low", torch.tensor([STEER_MIN, SPEED_MIN], device=device))
        self.register_buffer("action_high", torch.tensor([STEER_MAX, SPEED_MAX], device=device))
    ...
    def act(self, state):
        action_norm = self.actor(state)
        action_mean = self.action_low + (self.action_high - self.action_low) * action_norm
