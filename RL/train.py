...
def train():
    ...
    console.print(f"[RL-Training] [{timestamp()}] Training Environment Name: {env_name}", style="magenta")
    ...
    if os.path.exists(checkpoint_path) and os.path.exists(training_state_path):
        ...
        console.print(f"[RL-Training] [{timestamp()}] Loaded existing weights from {checkpoint_path}", style="green")
    
    else:
        start_time = datetime.now().replace(microsecond=0)
        console.print(f"[RL-Training] [{timestamp()}] Started training at (GMT) :", start_time, style="magenta")
        time_step = 0
        i_episode = 0
    ...
    while time_step <= max_training_timesteps:
        ...
        while not done:
            ...
            if time_step % print_freq == 0:
                ...
                console.print(f"[RL-Training] [{timestamp()}] Episode: {i_episode} Timestep: {time_step} Average Reward: {print_avg_reward}", style="yellow")
...
if __name__ == "__main__":
    try:
        ...
        console.print(f"[RL-Training] [{timestamp}] Training interrupted.", style="red")